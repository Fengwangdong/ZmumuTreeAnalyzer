...
label3 = ROOT.TLatex(0.55,0.86, "passOS prefit")
label1.SetNDC()
label2.SetNDC()
...
for k,keyHist in enumerate(histList):
    ...
    data = fData.Get("pass_prefit/data_obs")
    data.Sumw2()
    data.SetStats(0)
    ...
    for i,histKey in enumerate(MCfileList):
        ...
        iMC = fData.Get("pass_prefit/" + histKey)
        iMC.SetStats(0)
        iMC.Sumw2()
    ...
    ratio.Draw("EP")
    grSyst.Draw("2")
    canvas.SaveAs("controlPlots/" + keyHist + "_pass_prefit.pdf")
