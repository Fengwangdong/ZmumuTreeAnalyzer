...
label3 = ROOT.TLatex(0.55,0.86, "failOS postfit")
label1.SetNDC()
label2.SetNDC()
...
for k,keyHist in enumerate(histList):
    ...
    data = fData.Get("fail_postfit/data_obs")
    data.Sumw2()
    data.SetStats(0)
    ...
    for i,histKey in enumerate(MCfileList):
        ...
        iMC = fData.Get("fail_postfit/" + histKey)
        iMC.SetStats(0)
        iMC.Sumw2()
    ...
    pad2.cd()
    ratio.Draw("EP")
    canvas.SaveAs("controlPlots/" + keyHist + "_fail_postfit.pdf")
